...
set @dNew = DATEADD(DAY,2,@d); -- 2 gun gelmek

SELECT @d 'Date', @dNew 'New Date';

set @dNew = DATEADD(MONTH,6,@d); -- 6 ay gelmek

SELECT @d 'Date', @dNew 'New Date';

set @dNew = DATEADD(MONTH,-5,@d); -- 5 ay cixmaq

SELECT @d 'Date', @dNew 'New Date';

set @dNew = DATEADD(YEAR,-1,@d); -- 1 il cixmaq
...
SELECT 45 + '65' + 8.9 
SELECT '45' +7+ '65' + 8.9 --> en azi 1 eded int olmalidir
...
DECLARE @nv NVARCHAR(200) = 'code'; --> nvarchar 1 char - 2 byte
SELECT LEN(@nv) 'len', DATALENGTH(@nv) 'datalength'; --> 4 , 8
--> 4 (str uzunlugu)|  8 (byte)

DECLARE @v VARCHAR(200) = 'code'; --> nvarchar 1 char - 1 byte
SELECT LEN(@v) 'len', DATALENGTH(@v) 'datalength'; --> 4 , 4
...
SELECT LEN(@dt) 'dt-len', DATALENGTH(@dt) 'dt-datalength'; --> datetime 8 byte yer tutur
...
go --> go yazdiqda asagida yeniden eyni adli variableni yeniden adlandirmaq olur. yeni sehife acarmis kimi
...
SELECT CONVERT(datetime,@dStr,112) '112'; --> style = input code 
DECLARE @dStr2 VARCHAR(200) = '210316';
SELECT CONVERT(datetime,@dStr2,12) '12'; --> style = input code 
